...
export function climateMood(w: MonthClimate): ClimateMood {
  if (w.tmax_c <= 6) return "polar";
  if (w.tmax_c >= 28 && w.humidity >= 75) return "humid";
  if (w.tmax_c >= 22 && w.rain_days <= 6) return "sun";
  if (w.rain_days >= 10) return "rain";
